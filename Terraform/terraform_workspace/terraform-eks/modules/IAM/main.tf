...
#
# One role per component. Each role:
#   1. Has a trust policy scoped to a specific namespace:serviceaccount
#   2. Has a permission policy scoped to only what that component needs
#
# Pattern is identical for all — only the namespace, SA name, and
...
# ─── Helper: reusable trust policy per service account ────────────────────────
# Instead of copy-pasting the trust policy JSON 4 times,
# we use a data source to generate it with different SA names each time.
...
# Manages ALBs and NLBs when you create Ingress or Service=LoadBalancer
# ═══════════════════════════════════════════════════════════════════════════════
...
resource "aws_iam_policy" "lb_controller" {
  ...
  # This is the official AWS-provided policy for LB Controller
  # Source: https://raw.githubusercontent.com/kubernetes-sigs/aws-load-balancer-controller/main/docs/install/iam_policy.json
  # Kept inline here so no external fetch is needed at apply time
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
# ═══════════════════════════════════════════════════════════════════════════════
# ROLE 2 — Cluster Autoscaler
# Scales node groups up/down based on pending pods / underutilised nodes
# ═══════════════════════════════════════════════════════════════════════════════
...
resource "aws_iam_policy" "cluster_autoscaler" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        # Read-only — discover node groups and their current state
        Effect = "Allow"
        Action = [
      ...
      {
        # Write — actually scale the node group up or down
        Effect = "Allow"
        Action = [
        ...
        Resource = "*"
        # Scoped to only ASGs belonging to this cluster
        Condition = {
          StringEquals = {
...
# ═══════════════════════════════════════════════════════════════════════════════
# ROLE 3 — External Secrets Operator
...
resource "aws_iam_policy" "external_secrets" {
  ...
  description = "External Secrets Operator permissions"
    ...
    Statement = [
      {
        ...
        # Resource = "arn:aws:secretsmanager:ap-south-1:ACCOUNT_ID:secret:sagar-eks-prod-*"
      },
      {
        # For SSM Parameter Store (if used alongside Secrets Manager)
        Effect = "Allow"
        Action = [
          "ssm:GetParameter",
          "ssm:GetParameters",
